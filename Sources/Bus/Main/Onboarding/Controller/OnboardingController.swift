...
class OnboardingController : BaseController {
    ...
    let B_OnboardingView = B_OnboardingView.loadFromNib()
    ...
    override func loadView() {
        super.loadView()
        self.view = B_OnboardingView
    }
    
    override func viewDidLoad() {
        super.viewDidLoad()
        load()
        self.B_OnboardingView.viewState = .loading
    }
    ...
    private func load() {
        ...
        OnboardingModel.loadOnboarding(name: "onboarding_ios_buses", callback: { result in
            ...
            case .failure(let err):
                self.B_OnboardingView.viewState = .error(.init(style: .warning, title: err.errorDescription, onRetry: nil))
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let items: [Element] = self.model.map { boarding in
                let image = Element(content:
                                        B_OnboardingView.ViewState.Image(imageURL: boarding.imageURL ?? "")
                )
                
                let subtitle = boarding.subtitle.toAttributed(color: .textPrimary, font: .Body_17_Regular)
                let text = Element(content:
                                    B_OnboardingView.ViewState.Text(title: boarding.title, mainText: subtitle)
                )
                let tableState = OldState(model: SectionState(header: nil, footer: nil), elements: [image,text])
                return Element(content: B_OnboardingView.ViewState.ContentCell(items: tableState))
            }
            ...
            DispatchQueue.main.async {
                self.B_OnboardingView.viewState = .loaded(state)
            }
        }
    }
}
